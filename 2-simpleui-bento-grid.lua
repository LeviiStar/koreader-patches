...
_G.require = function(modname)
    ...
    -- =========================================================
    -- Appearance Plugin Crash Shield
    -- The Appearance plugin assumes it's always running inside a book and tries to style progress bars.
    -- Since SimpleUI draws bars on the homescreen, it throws nil errors. 
    -- Fix: Wrap the unsafe methods in a pcall (try/catch equivalent) so it fails silently instead of crashing.
    -- =========================================================
    if modname == "appearance.koplugin/book/progress_bar_colors" then
        ...
        if pw_ok and ProgressWidget and not ProgressWidget._bento_shield then
            ...
            if type(ProgressWidget.updateStyle) == "function" then
                ...
                ProgressWidget.updateStyle = function(self, ...)
                    local ok, err = pcall(unsafe_update, self, ...)
                    if not ok then original_require("logger").warn("Bento Shield blocked updateStyle crash:", err) end
                end
            end
            ...
            if type(ProgressWidget._setColors) == "function" then
                ...
                ProgressWidget._setColors = function(self, ...)
                    local ok, err = pcall(unsafe_set, self, ...)
                    if not ok then original_require("logger").warn("Bento Shield blocked _setColors crash:", err) end
                end
            end
    ...
    if modname == "sui_homescreen" and type(loaded) == "table" and loaded.show and not loaded._bento_patched then
        ...
        loaded.show = function(...)
                                    ...
                                    callback = function()
                                        ...
                                        UIManager:show(SpinWidget:new{
                                            ...
                                            callback      = function(spin)
                                                -- Save state and trigger a UI refresh
            ...
            -- 2. The Render Hook ("Ghost Injection")
            -- We monkey-patch the build functions just long enough to apply our scaling math, 
            -- then immediately restore them so other plugins don't freak out.
            ...
            inst._updatePage = function(self, keep_cache, books_only)
                ...
                -- Bail early if we're in landscape (grid looks terrible horizontally)
                ...
                -- Pre-fetch settings from storage so we have them ready during the synchronous render pass
                ...
                for _, mod in ipairs(modules) do
                    if type(mod.build) == "function" then
                        ...
                        mod.build = function(w, ctx)
                            ...
                            -- Subtract a few pixels to account for padding and prevent flexbox wrapping bugs
                ...
                self._makeModWrapper = function(this, mod, widget, w)
                    ...
                    if wrapped and type(wrapped) == "table" then
                        -- Tag the DOM elements so we know which module they belong to during repacking
                        wrapped._bento_mod_id = m_id
                        wrapped._bento_bw = bw
                        -- Force touch zones to shrink to the new column width
                        if wrapped.dimen then wrapped.dimen.w = bw end
                    end
                    return wrapped
                end

                -- Execute the core KOReader page build
                local ok, err = pcall(orig_updatePage, self, keep_cache, books_only)

                -- CLEANUP: Instantly restore original functions
                for _, mod in ipairs(modules) do
                ...
                if not ok then 
                    original_require("logger").warn("SimpleUI Bento Patch Error:", err)
                    return 
                end

                -- Abort if no modules built (means user is in the SimpleUI Library view, not the homescreen)
                if modules_built == 0 then return end

                -- 3. Masonry Repack Engine
                ...
                -- Extract widgets silently. DO NOT call .clear() here or the text buffers get destroyed.
                local children = {}
                ...
                for i, w in ipairs(children) do
                    if type(w) == "table" and w._bento_mod_id then
                        ...
                        -- Keep global headers/footers separate from the grid
                ...
                -- Push headers back into the DOM
                ...
                -- Helper to flush a completed flex row to the screen
                    ...
                    for i, col in ipairs(current_row_cols) do
                        ...
                        for _, chunk in ipairs(col.chunks) do
                            for _, w in ipairs(chunk.widgets) do
                                ...
                                -- Hardcap text widget widths to prevent them from stretching the column
                                ...
                                -- Maintain internal pointers for dynamic widgets (clock updates)
                        ...
                        -- Apply horizontal gutters between columns
                ...
                for _, chunk in ipairs(chunks) do
                    ...
                    if pct >= 1.0 then
                        -- 100% width items break the grid flow and get their own row
                        flush_row()
                        for _, w in ipairs(chunk.widgets) do 
                        ...
                        if current_row_width + pct <= 1.01 then
                            -- Fits on current row, append it horizontally
                            table.insert(current_row_cols, { width = pct, chunks = { chunk } })
                            current_row_width = current_row_width + pct
                        else
                            -- Doesn't fit horizontally. Try stacking it vertically under an existing column of the same width.
                            local stacked = false
                            ...
                            -- If we can't stack it, flush the row and start a new one
                            if not stacked then
                                flush_row()
                                table.insert(current_row_cols, { width = pct, chunks = { chunk } })
                ...
                -- Catch any leftover items
                flush_row()

                -- Push footers back into the DOM
                for _, w in ipairs(post_build_widgets) do self._body[#self._body + 1] = w end
            end

            -- FIX FOR THE 100% WIDTH FIRST BOOT GLITCH:
            -- Force SimpleUI to immediately bypass its cache and rebuild the DOM using our new math 
            -- before the UI thread has a chance to paint the screen!
